...
/*
 * XXX: This is an arbitrary (and too low) limit.  We need to look into dynamic
 * allocation of larger SGLs similar to either the NVMe PCIe host driver or the
 * SCSI midlayer.  And preferably unify the two methods while we're at it.
 */
#define NVME_RDMA_MAX_SEGMENTS		32
...
struct nvme_rdma_request {
	...
	struct scatterlist	sg[NVME_RDMA_MAX_SEGMENTS];
...
static int nvme_rdma_init_io_queues(struct nvme_rdma_ctrl *ctrl)
{
	...
	for (i = 1; i < ctrl->queue_count; i++) {
		ret = nvme_rdma_init_queue(ctrl, i, ctrl->queue_size);
		if (ret) {
			dev_info(ctrl->ctrl.dev,
		...
		struct request *rq)
{
	...
	ib_dma_unmap_sg(ibdev, req->sg, req->nents, rq_data_dir(rq) == WRITE ?
				DMA_TO_DEVICE : DMA_FROM_DEVICE);

}

static void nvme_rdma_encode_sgl(struct nvme_rsgl_desc *sg,
		u64 addr, u32 len, u32 key, u8 format)
{
	WARN_ON_ONCE(len > 0xffffff);
...
{
	req->sge[1].addr = ib_sg_dma_address(queue->device->dev, req->sg);
...
#if 0
static int nvme_rdma_map_sg_phys(struct nvme_rdma_queue *queue,
		struct nvme_rdma_request *req, struct nvme_command *c,
		int count)
{
	size_t offset = sizeof(*c);
	struct nvme_rsgl_desc *ind =
		(struct nvme_rsgl_desc *)((void *)c + offset);
	struct scatterlist *sg;
	int ndesc = 0, i;

	for_each_sg(req->sg, sg, count, i) {
		nvme_rdma_encode_sgl(ind,
				ib_sg_dma_address(queue->device->dev, sg),
				ib_sg_dma_len(queue->device->dev, sg),
				queue->device->mr->rkey,
				NVME_SGL_FMT_DATA_DESC << 4);
		ind++;
		ndesc++;
	...
	nvme_rdma_encode_sgl(&c->common.rsgl, offset,
			ndesc * sizeof(struct nvme_rsgl_desc), 0,
		...
		struct request *rq, struct nvme_command *c)
{
	...
	sg_init_table(req->sg, rq->nr_phys_segments);
	nents = blk_rq_map_sg(rq->q, rq, req->sg);
	BUG_ON(nents > rq->nr_phys_segments);
	req->nents = nents;

	count = ib_dma_map_sg(ibdev, req->sg, nents, rq_data_dir(rq) == WRITE ?
				DMA_TO_DEVICE : DMA_FROM_DEVICE);
	if (unlikely(count <= 0))
		return -EIO;
...
static int __nvme_rdma_recv_done(struct ib_cq *cq, struct ib_wc *wc, int tag)
{
	...
	ret = nvme_rdma_process_nvme_rsp(queue, qe->data, wc, tag);
		...
		const struct blk_mq_queue_data *bd)
{
	...
	case REQ_TYPE_FS:
		if (rq->cmd_flags & REQ_FLUSH)
			nvme_setup_flush(ns, c);
		else
			nvme_setup_rw(ns, rq, c);
		break;
	case REQ_TYPE_DRV_PRIV:
		memcpy(c, rq->cmd, sizeof(struct nvme_command));
	...
	ret = nvme_rdma_post_send(queue, sqe, req->sge, req->num_sge,
			req->need_inval ? &req->reg_wr.wr : NULL);
	if (ret) {
...
static int nvme_rdma_configure_admin_queue(struct nvme_rdma_ctrl *ctrl)
{
	...
	ctrl->admin_tag_set.queue_depth = NVMF_AQ_DEPTH;
	...
	aqa = NVMF_AQ_DEPTH - 1;
	aqa |= aqa << 16;
	error = nvmf_reg_write32(&ctrl->ctrl, NVME_REG_AQA, aqa);
	if (error) {
		dev_err(ctrl->ctrl.dev,
			"prop_set NVME_REG_ACA failed\n");
		goto out_cleanup_queue;
	}
