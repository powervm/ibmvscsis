...
static void ibmveth_rx_csum_helper(struct sk_buff *skb, bool is_active_trunk) 
{
	 ...
	 */
	if (iph_proto == IPPROTO_TCP && is_active_trunk) {
		struct tcphdr *tcph = (struct tcphdr *)(skb->data + iphlen);
		tcphdrlen = skb->len - iphlen;

		if (skb_proto == ETH_P_IP)
			tcph->check = ~csum_tcpudp_magic(iph->saddr, iph->daddr,
					 tcphdrlen, iph_proto, 0);
		else if (skb_proto == ETH_P_IPV6)
			tcph->check = ~csum_ipv6_magic(&iph6->saddr, &iph6->daddr,
					 tcphdrlen, iph_proto, 0);

		/* Setup SKB fields for checksum offload */
		skb_partial_csum_set(skb, iphlen, offsetof(struct tcphdr, check));
	...
	while (frames_processed < budget) {
		...
		if (!ibmveth_rxq_buffer_valid(adapter)) {
			...
			if (csum_good) {
				...
				ibmveth_rx_csum_helper(skb, adapter->is_active_trunk);
			}
